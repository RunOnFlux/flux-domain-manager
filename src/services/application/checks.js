...
async function isVersionOK(ip, port) {
  try {
    ...
    if (minVersionSatisfy(version, '5.13.2')) {
...
function kadenaCheckHeight(height) {
  ...
  const baseTime = 1733996360000;
  const baseHeight = 107683420;
  const timeDifference = currentTime - baseTime;
...
async function checkBlockBook(ip, port, appsname) {
  try {
    const coinList = ['litecoin', 'flux', 'ethereumclassic', 'vertcoin', 'zcash', 'dogecoin', 'digibyte', 'groestlcoin', 'dash', 'firo', 'sin', 'ravencoin', 'pivx', 'decred', 'neurai', 'bitcoin', 'bitcointestnet', 'bitcoinsignet', 'clore', 'bitcoincash'];
    const addressList = ['LVjoCYFESyTbKAEU5VbFYtb9EYyBXx55V5', 't3fK9bY31MGCqhKw34cg9gg168SHCfcMGHe', '0x0e009d19cb4693fcf2d15aaf4a5ee1c8a0bb5ecf', 'VbFrQgNEiR8ZxMh9WmkjJu9kkqjJA6imdD',
      ...
      'AMq8KfE2iJtMbKNMtHp3VmJFFKmyLoMwuG', '12ib7dApVFvg82TXKycWBNpN8kFyiAN1dr'];
    const heightList = [2561528, 1489960, 18510512, 2067081, 2260134, 4922428, 18038850, 4796068, 1953740, 764150, 1690368, 3015843, 4085836, 807730, 255116, 812896, 2534408, 165752, 516509, 845000];
